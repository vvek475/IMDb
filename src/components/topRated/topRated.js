...
export default function TopRated({moviearray,url}){
    ...
    return(
        ...
        <Link to={`/page/${url}`} ><h2 className="slide__title">Top Rated<span className="fancy" children=" >>"></span></h2></Link>
            ...
            {values && more && (values.map((movies)=>(
                <span className="movie_scroll">
                    ...
                    <Link to={`/movieInfo/${movies.id}`} >
                        <p className="toprated__title">{movies.title}</p>
                    </Link>
                    <p><span className="star">★ </span> {movies.vote_average}</p>
                    <p className="toprated__release">Release : {movies.release_date}</p>
                    </div>
                </span>
            )))}
